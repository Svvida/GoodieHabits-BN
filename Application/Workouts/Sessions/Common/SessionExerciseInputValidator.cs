...
{
    public class SessionExerciseInputValidator : AbstractValidator<SessionExerciseInput>
    {
        public SessionExerciseInputValidator()
        {
            ...
            RuleFor(i => i.Note)
                .MaximumLength(WorkoutSessionExercise.NoteMaxLength).When(i => i.Note is not null)
                .WithMessage("Note must not exceed {MaxLength} characters.");

            RuleForEach(i => i.Sets).SetValidator(new SessionSetInputValidator());
        }
    }
}
